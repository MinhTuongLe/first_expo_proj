...
export default {
  ...
  setAsyStrSettingsLocal: async (value) => {
    try {
  ...
  setAsyStrShowNameChoosed: async (value) => {
    try {
  ...
  /** Set async storage for class choosed */
  setAsyStrClassChoosed: async (value) => {
    try {
  ...
  setAsyStrStudentChoosed: async (value) => {
    try {
   ...
   */
  capitalizeName: (firstName, lastName, option = "softname_first_last") => {
  ...
  resetNavigation: (navigation, routeName, params = null) => {
  ...
  /* Format datetime */
  getShortTimeWithNow: (timestamp) => {
    try {
      let _tmpTime = moment.unix(timestamp / 1000);
      let _tmpNow = moment();
      ...
      if (compare > 0) {
        ...
        return {
          ...
          des: moment
            .unix(timestamp / 1000)
            .format(CONFIG.formatDateSetting ?? CONFIG.formatDate),
        };
      } else if (compare == 0) {
  ...
  parseTimeNews: (time) => {
    let diffYear = moment().diff(
      moment(time, `${CONFIG.formatDateSetting ?? CONFIG.formatDate}THH:mm:ss`),
      "years"
    );
    if (diffYear > 0) {
    ...
    if (diffYear === 0) {
      let diffMonth = moment().diff(
        moment(
          time,
          `${CONFIG.formatDateSetting ?? CONFIG.formatDate}THH:mm:ss`
        ),
        "months"
      );
      if (diffMonth > 0) {
  ...
  animTiming: (state, toValue, duration, isStart) => {
    if (isStart) {
