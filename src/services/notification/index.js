...
export default {
  notRead: (params) => {
    try {
      let newUrl = sailsApi.notification.notRead + '/' + params.id + '?school=' + params.school ;
  ...
  list: (params) => {
    try {
      let newUrl = sailsApi.notification.list + '?limit=' + params.limit + '&page=' + params.page + '&id=' + params.id + '&school=' + params.school;
      let results = Api.get(newUrl, CONFIG.versionAPI);
      return results;
    } catch (error) {
      console.log('ERROR ASYNC: ', error);
