...
export default {
	listCMS: (per_page = 10, page = 1, category = null, author = null, tag = null) => {
		try {
	...
	listCMSPopular: () => {
		try {
			let results = ApiZini.get(WPApi.popularPosts.listPosts);
			return results;
		} catch (error) {
			return null;
		}
	...
	list: (params) => {
		try {
			let newUrl = sailsApi.news.list + '?limit=' + params.limit + '&page=' + params.page + '&schoolId=' + params.school;
